...
YUI().use('event-focus', 'json', 'model', 'model-list', 'view', function (Y) {
	...
	function changeWord(){
		var urlForGet = "http://www.ishahar.co.il/yahoo/get.php?user_ID=1";
		if(vs_round_ID) 
        {
            vs = true;
	...
	/*function doAjaxQueryResults() {// this function will handle the processing
	...
	function ajaxStateChangeResult(){
		...
		if (ajaxObject.readyState == 4) {// if ready state is 4 (the page is finished loading)
			if (ajaxObject.status == 200) {// if the status code is 200 (everything's OK)
				...
				score += jasonObj.score;
				
				var bodyString = "<table align='center' cellpadding='10px'><tr><td style='text-align: center;'>Results:</td></tr><tr><td style='text-align: center;'>Score: " + score + "</td></tr>";
				
				for(var i=0;i<jasonObj.answers.length;i++){
					if(jasonObj.answers[i].correct == false){
						bodyString += "<tr>";
						bodyString += "<td style='color: red; font-weight: bold;'> X ";
						bodyString += jasonObj.answers[i].name;	
						bodyString += "</td>";
						bodyString += "</tr>";
					}
					else{
						...
						bodyString += "<tr>";	
						var track = jasonObj.answers[i].track_title;
						var album = jasonObj.answers[i].album_name;
						var artist = jasonObj.answers[i].artist_name;
						var link = jasonObj.answers[i].track_share_url;
						var cover = jasonObj.answers[i].album_cover;
						
						bodyString += "<td style='color: green; font-weight: bold;'>";
						if(track != null) bodyString += track;
						bodyString += "</td>";
						bodyString += "<td>";
						if(artist != null) bodyString += artist;
						bodyString += "</td>";
						bodyString += "<td>";
						if(album != null) bodyString += album;
						bodyString += "</td>";
						bodyString += "<td>";
						bodyString += "<a href='" + link + "'>";
						if(cover != null)bodyString += "<img src='" + cover + "'></img>";
						bodyString += "</a></td>";
						bodyString += "</tr>";
				...
				bodyString += "<tr><td><button onclick='window.location.reload();'>Restart</button></td></tr></table> ";
				
				document.body.innerHTML = bodyString;
				//document.body.innerHTML = "kaka";
				//document.getElementById("results").setAttribute("style","background-color: blue;");
			}// end if
			else {// if the status code is anything else (bad news)
				alert('There was an error. HTTP error code ' + ajaxObject.status.toString() + '.');
				return;
